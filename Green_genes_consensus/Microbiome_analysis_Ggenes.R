...
setwd("/DATA/home/jmlazaro/Projects/Microbiome/FINAL_REF")

#Reading FASTA Files
gg_13_Assembly_Microbiome <- readDNAStringSet("gg_13_5.fasta.gz")
HMCQCP_Assembly_Microbiome_rep <-readDNAStringSet("rep_set_v13.fna")
HMCQCP_Names <- names(HMCQCP_Assembly_Microbiome_rep)
HMCQCP_Names <- gsub(" .*","",HMCQCP_Names)
names(HMCQCP_Assembly_Microbiome_rep) <- HMCQCP_Names
...
BLAST_Assembly_Microbiome <- readDNAStringSet("16S_ribosomal_BLAST.fasta")
BLAST_Names_info <- names(BLAST_Assembly_Microbiome)

BLAST_Names <- gsub(" .*","",BLAST_Names_info)
BLAST_Species <-  gsub(".*1 ","",BLAST_Names_info)
names(HMCQCP_Assembly_Microbiome_rep) <- HMCQCP_Names



Microbiome_Assembly_Genbank <- read.table("gg_13_5_accessions.txt", header = TRUE)
best_single_hits.blastn <- read.table("best_single_hits_gg13.blastn")
colnames(best_single_hits.blastn) <- c("qseqid","gg_id","pident","length","mismatch","gapopen","qstart","qend","sstart","send","evalue","bitscore")


HMQCP_gg_13_merge <- merge(best_single_hits.blastn,Microbiome_Assembly_Genbank, all.x = TRUE, by = "gg_id")
